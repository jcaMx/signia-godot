...
func process_sign(sign: String) -> String:
	return get_challenge_target(sign)


func normalize_sign_id(sign: String) -> String:
	return String(sign).strip_edges().to_upper()


func get_challenge_type(sign: String) -> String:
	var normalized_sign = normalize_sign_id(sign)
...
func get_challenge_target(sign: String) -> String:
	var normalized_sign = normalize_sign_id(sign)
	if normalized_sign.begins_with(SPELL_PREFIX):
